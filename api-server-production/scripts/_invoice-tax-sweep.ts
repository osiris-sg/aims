...
async function main() {
  TK = await tokens();
  const invs = await prisma.document.findMany({ where: { organizationId: ORG, type: { in: ['INVOICE', 'CREDIT_NOTE'] }, OR: [{ name: { startsWith: 'BIPL-JPSG' } }, { name: { startsWith: 'JPINV' } }, { name: 'BI202607106' }] }, select: { name: true, config: true } });
  let checked = 0, taxed = 0;
  for (const i of invs) {
    const c: any = i.config || {};
    const id = c.xeroInvoiceId || c.xeroCreditNoteId;
    if (!id || ['DELETED', 'VOIDED'].includes(c.xeroStatus)) continue;
    try {
      const g = await xero('GET', `/${c.xeroCreditNoteId ? 'CreditNotes' : 'Invoices'}/${id}`);
      const doc = (g.Invoices || g.CreditNotes)[0];
      checked++;
      if (Number(doc.TotalTax || 0) > 0.005) { taxed++; console.log(`  ! ${i.name}: TotalTax=${doc.TotalTax}`); }
    } catch {}
    await sleep(1100);
  }
  console.log(`invoice tax sweep: checked=${checked} with-GST=${taxed} (expected 0)`);
