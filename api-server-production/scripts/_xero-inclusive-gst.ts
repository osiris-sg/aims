...
async function main() {
  ...
  const targets = bills.filter(b => { const c: any = b.config || {}; return !((c.reference || '')).startsWith('BIPL-JPSG') && c.xeroBillId; });
  console.log(`targets: ${targets.length}`);
  let ok = 0, failed = 0, skipped = 0;
  for (let i = 0; i < targets.length; i += 30) {
    const chunk = targets.slice(i, i + 30);
    const ids = chunk.map(b => (b.config as any).xeroBillId).join(',');
    const res = await xero('GET', `/Invoices?IDs=${encodeURIComponent(ids)}`);
    const toPost: any[] = [];
    for (const inv of res.Invoices || []) {
      if (inv.Status !== 'DRAFT') { skipped++; console.log(`  = skip ${inv.InvoiceNumber} (${inv.Status})`); continue; }
      toPost.push({
        InvoiceID: inv.InvoiceID,
        LineAmountTypes: 'Inclusive',
        LineItems: (inv.LineItems || []).map((l: any) =>
          Number(l.LineAmount) > 0
            ? { LineItemID: l.LineItemID, Description: l.Description, Quantity: l.Quantity, UnitAmount: l.UnitAmount, AccountCode: l.AccountCode, TaxType: 'INPUTY24' }
    ...
    if (toPost.length) {
      ...
      for (const r of up.Invoices || []) {
        if (r.HasErrors) { failed++; console.log(`  x ${r.InvoiceNumber}: ${r.ValidationErrors?.[0]?.Message}`); }
        else { ok++; if (ok === 1) console.log(`  first ok: ${r.InvoiceNumber} total=${r.Total} tax=${r.TotalTax}`); }
      }
      await sleep(1100);
    }
  }
  console.log(`DONE xero inclusive-GST: ok=${ok} failed=${failed} skipped=${skipped}`);
