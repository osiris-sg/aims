...
async function main() {
  TK = await tokens();
  const NAMES = ['BIPL-JPSG-INV-20260515-0002', 'BIPL-JPSG-INV-20260523-0005', 'BIPL-JPSG-INV-20260526-0066', 'BIPL-JPSG-INV-20260721-0152'];
  for (const name of NAMES) {
    ...
    if (!c.xeroCreditNoteId) { console.log(`${name}: not in xero`); continue; }
    const g = await xero('GET', `/CreditNotes/${c.xeroCreditNoteId}`);
    const cn = g.CreditNotes[0];
    console.log(`${name}: ${cn.Status} $${cn.Total} allocated=${cn.Allocations?.length || 0} remaining=${cn.RemainingCredit}`);
    await prisma.document.update({ where: { id: d!.id }, data: { config: { ...c, xeroStatus: cn.Status } } });
    await sleep(1100);
  }
