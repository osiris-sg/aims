...
async function main() {
  TK = await tokens();
  const res = await xero('GET', `/Invoices?where=${encodeURIComponent('InvoiceNumber.StartsWith("BIPL-JPSG-INV-20260708-0089")')}`);
  for (const inv of res.Invoices || []) console.log(`"${inv.InvoiceNumber}" id=${inv.InvoiceID} ${inv.Status} $${inv.Total} tax=${inv.TotalTax}`);
  const d = await prisma.document.findFirst({ where: { organizationId: ORG, type: 'INVOICE', name: 'BIPL-JPSG-INV-20260708-0089' }, select: { config: true } });
  console.log('AIMS stored id:', (d!.config as any).xeroInvoiceId);
