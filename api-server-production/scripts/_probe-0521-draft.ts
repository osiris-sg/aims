...
async function main() {
  TK = await tokens();
  const g = await xero('GET', '/Invoices/20613639-5543-4cac-ac84-0958d09ea475');
  const inv = g?.Invoices?.[0];
  console.log(`live draft: total=${inv?.Total} date=${inv?.DateString} contact=${inv?.Contact?.Name} ref="${inv?.Reference}"`);
  (inv?.LineItems || []).forEach((l: any) => console.log(`  line: qty=${l.Quantity} unit=${l.UnitAmount} amt=${l.LineAmount} acct=${l.AccountCode} "${(l.Description || '').slice(0, 80)}"`));
