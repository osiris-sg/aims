// Zero-amount text lines (refs, headers, cables specs, project blocks) get NO
// quantity — blank, not 1 (guru 2026-08-25, print-view feedback).
...
(async () => {
  ...
  for (const d of ours) {
    ...
    const items = (c.items || []).map((it: any) => {
      ...
      if (amt === 0 && up === 0 && it.quantity != null) { dirty = true; lines++; return { ...it, quantity: null }; }
      return it;
    });
    if (dirty) { changed++; await prisma.document.update({ where: { id: d.id }, data: { config: { ...c, items } } }); }
  }
  console.log(`blanked qty on ${lines} text lines across ${changed}/${ours.length} drafts`);
  process.exit(0);
})();
