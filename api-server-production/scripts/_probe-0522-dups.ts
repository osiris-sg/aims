...
async function main() {
  TK = await tokens();
  const res = await xero('GET', `/Invoices?where=${encodeURIComponent('InvoiceNumber.StartsWith("BIPL-JPSG-INV-20260522-0003")')}`);
  for (const inv of res.Invoices || []) {
    console.log(`"${inv.InvoiceNumber}" id=${inv.InvoiceID.slice(0, 8)} ${inv.Status} $${inv.Total} paid=$${inv.AmountPaid} date=${inv.DateString?.slice(0, 10)}`);
  }
