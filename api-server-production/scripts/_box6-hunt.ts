...
const PC = new Set(["1", "8", "10"]); // OUTPUT codes
(async () => {
    ...
    for (let page = 1; ; page++) {
      ...
      for (const doc of rows) {
        ...
        for (const li of doc.LineItems || []) {
          ...
          if (!m || m.side !== "OUTPUT" || !PC.has(m.code)) continue;
          addX((doc[numKey] || "").trim(), R((Number(li.TaxAmount) || 0) * cnSign));
        }
      }
  ...
  for (const doc of docs) {
    ...
    if (typed.length) {
      for (const it of typed) {
        ...
        if (m.side !== "OUTPUT" || !PC.has(m.code)) continue;
        addA(key, R(-(Number(it.taxAmount) || 0) * signFor(m.side)));
      }
      continue;
    }
