...
async function main() {
  TK = await tokens();
  // bill numbers may carry suffixes ("JPxxx (EMPLOYER)" / "JPxxx · BI...") — search both exact and prefix
  const exact = await xero('GET', `/Invoices?where=${encodeURIComponent('Type=="ACCPAY" AND InvoiceNumber=="JP2607170118"')}`);
  console.log('exact matches:', (exact.Invoices || []).length);
  for (const inv of exact.Invoices || []) console.log(`  ${inv.InvoiceID} status=${inv.Status} total=${inv.Total}`);
  await sleep(1100);
  const pre = await xero('GET', `/Invoices?where=${encodeURIComponent('Type=="ACCPAY" AND InvoiceNumber.StartsWith("JP2607170118")')}`);
  console.log('prefix matches:', (pre.Invoices || []).length);
  for (const inv of pre.Invoices || []) console.log(`  "${inv.InvoiceNumber}" status=${inv.Status} total=${inv.Total} contact=${inv.Contact?.Name}`);
