...
async function main() {
  TK = await tokens();
  const d = await prisma.document.findFirst({ where: { organizationId: ORG, type: 'CREDIT_NOTE', name: 'BIPL-JPSG-INV-20260721-0152' }, select: { config: true } });
  const c: any = d!.config || {};
  const g = await xero('GET', `/CreditNotes/${c.xeroCreditNoteId}`);
  const cn = g.CreditNotes[0];
  console.log(`updatedUTC=${cn.UpdatedDateUTC}`);
  for (const a of cn.Allocations || []) {
    console.log(`allocation: $${a.Amount} on ${a.Date} → invoice ${a.Invoice?.InvoiceNumber || a.Invoice?.InvoiceID}`);
  }
  for (const p of cn.Payments || []) console.log(`payment: $${p.Amount} ${p.Date}`);
