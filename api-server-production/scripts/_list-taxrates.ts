...
async function main() {
  TK = await tokens();
  const res = await xero('GET', '/TaxRates');
  for (const t of res.TaxRates || []) {
    if (t.Status !== 'ACTIVE') continue;
    console.log(`${t.TaxType.padEnd(10)} ${String(t.EffectiveRate).padStart(5)}%  expenses=${t.CanApplyToExpenses}  ${t.Name}`);
  }
