...
async function main() {
  TK = await tokens();
  for (const num of ['BIPL-JPSG-INV-20260524-0001', 'BIPL-JPSG-INV-20260715-0047']) {
    const res = await xero('GET', `/Invoices?InvoiceNumbers=${encodeURIComponent(num)}`);
    console.log(`\n${num}: ${(res.Invoices || []).length} document(s) in Xero`);
    for (const inv of res.Invoices || []) {
      console.log(`  id=${inv.InvoiceID} ${inv.Status} $${inv.Total} paid=$${inv.AmountPaid} contact=${inv.Contact?.Name} date=${inv.DateString?.slice(0, 10)}`);
    }
    await sleep(1100);
  }
