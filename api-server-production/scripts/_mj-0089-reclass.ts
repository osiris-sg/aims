...
async function main() {
  TK = await tokens();
  // $20 reclass: move 0708-0089's recharge credit out of 105 Contra into 443,
  // dated on the invoice date, without touching the reconciled payment.
  const res = await xero('PUT', '/ManualJournals', {
    ManualJournals: [{
      Narration: 'Reclass: BIPL-JPSG-INV-20260708-0089 recharge credit from Contra (105) to JP Pass Application External (443) — middleman clearing',
      Date: '2026-07-08',
      Status: 'POSTED',
      JournalLines: [
        { LineAmount: 20.0, AccountCode: '105' },
        { LineAmount: -20.0, AccountCode: '443' },
      ],
    }],
  });
  const mj = res.ManualJournals?.[0];
  console.log(mj?.HasErrors ? `ERR: ${JSON.stringify(mj.ValidationErrors).slice(0, 200)}` : `manual journal posted ✓ (${mj.ManualJournalID})`);
