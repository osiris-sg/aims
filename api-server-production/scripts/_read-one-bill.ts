...
async function main() {
  ...
  const g = await xero('GET', `/Invoices/${(b!.config as any).xeroBillId}`);
  const inv = g.Invoices[0];
  console.log(`status=${inv.Status} LineAmountTypes=${inv.LineAmountTypes} Total=${inv.Total} Tax=${inv.TotalTax} SubTotal=${inv.SubTotal} paid=${inv.AmountPaid} due=${inv.AmountDue}`);
  (inv.LineItems || []).forEach((l: any) => console.log(`  line: qty=${l.Quantity} unit=${l.UnitAmount} amt=${l.LineAmount} tax=${l.TaxAmount} type=${l.TaxType} acct=${l.AccountCode}`));
