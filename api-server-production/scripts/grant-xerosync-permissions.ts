/**
 * Grant xerosync:* permissions to superadmin + Admin roles in every org.
 * Idempotent.
 */
...
const PERMS = [
  { resource: 'xerosync', action: 'read', description: 'View Xero connection status + sync runs + mappings' },
  { resource: 'xerosync', action: 'create', description: 'Run sync + edit mappings + create AIMS accounts from Xero' },
];

async function main() {
  console.log('🔐 Ensuring xerosync:* permissions...');
  const perms = [];
