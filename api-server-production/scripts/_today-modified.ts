...
async function main() {
  ...
  for (const page of [1, 2]) {
    let res: any;
    try {
      res = await fetch(`${XERO_API}/Invoices?page=${page}&summaryOnly=true`, { headers: { Authorization: `Bearer ${TK.at}`, 'Xero-Tenant-Id': TK.tid, Accept: 'application/json', 'If-Modified-Since': 'Tue, 29 Jul 2026 00:00:00 GMT' } }).then(r => r.status === 304 ? { Invoices: [] } : r.json());
    } catch (e) { break; }
    const invs: any[] = res.Invoices || [];
    if (!invs.length) break;
    for (const inv of invs) {
      console.log(`${inv.InvoiceNumber} · ${inv.Type} · ${inv.Status} · $${inv.Total} · updated=${new Date(Number(inv.UpdatedDateUTC.match(/\d+/)[0])).toISOString().slice(0, 16)}`);
    }
    await sleep(1100);
  }
