...
(async () => {
  ...
  const r = await fetch("https://api.xero.com/api.xro/2.0/TaxRates", { headers: { Authorization: `Bearer ${TK.at}`, "Xero-Tenant-Id": TK.tid, Accept: "application/json" } });
  const j: any = await r.json();
  for (const t of j.TaxRates || []) if (t.Status === "ACTIVE") console.log(`${t.TaxType.padEnd(10)} ${String(t.EffectiveRate).padStart(6)}%  ${t.Name}  ${t.CanApplyToExpenses ? "[expenses]" : ""}`);
  process.exit(0);
})();
