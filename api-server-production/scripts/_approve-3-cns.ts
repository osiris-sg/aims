...
async function xero(method: string, path: string, body?: any) {
  for (let i = 0; i < 6; i++) {
    ...
    if (!res.ok) throw new Error(`${res.status}: ${JSON.stringify(json).slice(0, 1500)}`);
    return json;
  }
...
async function main() {
  TK = await tokens();
  const NAMES = ['BIPL-JPSG-INV-20260515-0002', 'BIPL-JPSG-INV-20260523-0005', 'BIPL-JPSG-INV-20260526-0066'];
  for (const name of NAMES) {
    const d = await prisma.document.findFirst({ where: { organizationId: ORG, type: 'CREDIT_NOTE', name }, select: { id: true, config: true } });
    const c: any = d!.config || {};
    if (!c.xeroCreditNoteId) { console.log(`x ${name}: no xeroCreditNoteId`); continue; }
    if (c.xeroStatus === 'AUTHORISED') { console.log(`= ${name} already authorised`); continue; }
    const res = await xero('POST', `/CreditNotes/${c.xeroCreditNoteId}`, { CreditNotes: [{ CreditNoteID: c.xeroCreditNoteId, Status: 'AUTHORISED' }] });
    const cn = res?.CreditNotes?.[0];
    if (cn?.HasErrors) { console.log(`x ${name}: ${cn.ValidationErrors?.[0]?.Message}`); continue; }
    await prisma.document.update({ where: { id: d!.id }, data: { config: { ...c, xeroStatus: 'AUTHORISED' } } });
    console.log(`ok ${name} → AUTHORISED ($${c.totals?.total ?? c.nettTotal})`);
    await sleep(1100);
  }
