// Push September AIMS invoices (BI202609NNN) to Xero as DRAFTs via app2.
// Idempotent (pre-checks InvoiceNumbers), validates account codes + totals.
// Usage: --only=BI202609001 | --from=BI202609001 --to=BI202609093 [--dry]
...
const ONLY = arg("only");
const FROM = arg("from") || "BI202609001";
const TO = arg("to") || "BI202609099";
...
async function xero(method: string, path: string, body?: any) {
  for (let i = 0; i < 6; i++) {
    ...
    if (res.status === 429) {
      ...
      if (w > 600) throw new Error(`DAILY_CAP:${w}`);
...
(async () => {
  ...
  const all = await prisma.document.findMany({ where: { organizationId: ORG, type: "INVOICE", name: ONLY ? ONLY : { gte: FROM, lte: TO } }, orderBy: { name: "asc" } });
  const docs = all.filter(d => /^BI202609\d{3}$/.test(d.name!));
  console.log(`${docs.length} candidate doc(s) · output tax ${OUTPUT_TAX} · due ${DUE_DATE}`);
  ...
  if (existing.size) console.log(`${existing.size} already in Xero (skipping)`);
  ...
  if (REFRESH) {
    for (let i = 0; i < docs.length; i += 40) {
      const nums = docs.slice(i, i + 40).map(d => d.name).join(",");
      const r = await xero("GET", `/Invoices?InvoiceNumbers=${encodeURIComponent(nums)}&summaryOnly=true`);
      for (const inv of r.Invoices || []) if (inv.Status === "DRAFT") existingById.set(inv.InvoiceNumber, inv.InvoiceID);
    }
  }
  const payloads: any[] = []; const skipped: string[] = [];
  for (const d of docs) {
    if (existing.has(d.name!) && !(REFRESH && existingById.has(d.name!))) continue;
    ...
    for (const it of amtLines) {
      if (!it.accountCode) probs.push(`line missing accountCode: "${(it.description || "").split("\n")[0].slice(0, 35)}"`);
      else if (!validCodes.has(it.accountCode)) probs.push(`account ${it.accountCode} not active in Xero`);
    }
    const sum = Math.round(items.reduce((s, it) => s + (Number(it.amount) || 0), 0) * 100) / 100;
    if (Math.abs(sum - (c.subTotal ?? sum)) > 0.02) probs.push(`line sum ${sum} != subTotal ${c.subTotal}`);
    if (Math.abs((Number(c.subTotal) + Number(c.gstAmount)) - Number(c.nettTotal)) > 0.02) probs.push("sub+gst != nett");
    if (!(Number(c.gstAmount) > 0)) probs.push("gstAmount is 0 — would push a GST-free invoice");
    ...
    const LineItems = items.map(it => {
      ...
      if (amt === 0 && !(Number(it.unitPrice) || 0)) {
        ...
        // (BI202607027). A pure annotation line has NO quantity and prints
      ...
      return {
        ...
        ...(it.itemCode ? { ItemCode: undefined } : {}),
      };
    });
    payloads.push({
      _docId: d.id, _nett: c.nettTotal,
      ...(existingById.has(d.name!) ? { InvoiceID: existingById.get(d.name!) } : {}),
      Type: "ACCREC",
      Contact: { ContactID: cust!.xeroId },
      InvoiceNumber: d.name,
      Reference: (c.reference || "").slice(0, 255),
      Date: (typeof c.date === "string" ? c.date : "2026-09-01").slice(0, 10),
      DueDate: DUE_DATE,
      Status: "DRAFT",
  ...
  if (DRY) { for (const p of payloads.slice(0, 2)) console.log(JSON.stringify(p, null, 1).slice(0, 1800)); console.log("[DRY] nothing pushed"); process.exit(0); }
  ...
  for (let i = 0; i < payloads.length; i += 20) {
    ...
    for (let k = 0; k < (r.Invoices || []).length; k++) {
      ...
      if (res.ValidationErrors?.length || res.HasErrors) { fail++; console.log(`  ✗ ${src.InvoiceNumber}: ${(res.ValidationErrors || []).map((e: any) => e.Message).join("; ").slice(0, 200)}`); continue; }
      ok++;
      const warn = Math.abs((res.Total ?? 0) - src._nett) > 0.02 ? `  ⚠ Xero total ${res.Total} vs AIMS ${src._nett}` : "";
      ...
      await prisma.document.update({ where: { id: src._docId }, data: { config: { ...cfg, xeroInvoiceId: res.InvoiceID, xeroInvoiceNumber: res.InvoiceNumber, xeroStatus: "DRAFT", xeroGross: res.Total, xeroSyncedAt: new Date().toISOString(), xeroSyncedBy: "app2-sept-push" } } });
    }
  }
  console.log(`\nDONE: ${ok} pushed as DRAFT, ${fail} failed, ${skipped.length} skipped`);
  process.exit(0);
})().catch(e => {
  if (String(e?.message).startsWith("DAILY_CAP:")) console.error(`\n⛔ Xero daily cap active (retry-after ${e.message.split(":")[1]}s). Re-run later — idempotent.`);
