...
async function main() {
  ...
  const refd = bills.filter(b => { const c: any = b.config || {}; return /^(BIPL-JPSG-INV|JPINV-|BI\d)/.test(c.reference || ''); });
  let unposted = 0; const unpostedList: string[] = [];
  for (const b of refd) {
    const c: any = b.config || {};
    if (!c.xeroBillId) { unposted += Number(c.totalAmount || 0); unpostedList.push(`${b.name}($${c.totalAmount}, not in Xero)`); continue; }
    if ((c.xeroStatus || 'DRAFT') !== 'DRAFT') continue;
    const g = await xero('GET', `/Invoices/${c.xeroBillId}`);
    const inv = g.Invoices[0];
    await sleep(1100);
    await prisma.document.update({ where: { id: b.id }, data: { config: { ...c, xeroStatus: inv.Status } } });
    if (inv.Status === 'DRAFT') { unposted += Number(c.totalAmount || 0); unpostedList.push(`${b.name}($${c.totalAmount}, draft)`); }
  }
  console.log(`mirrors synced. cost NOT yet posted to 443: $${unposted.toFixed(2)}`);
  unpostedList.forEach(x => console.log('  ' + x));
