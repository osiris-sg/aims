...
async function main() {
  ...
  const paid = docs.filter(d => { const c: any = d.config || {}; return c.xeroSyncedBy === 'jpsg-push' && c.xeroStatus === 'PAID'; }).slice(0, 6);
  for (const d of paid) {
    ...
    for (const p of inv.Payments || []) {
      ...
      const pay = pg.Payments[0];
      console.log(`${d.name} $${p.Amount} on ${p.Date?.match(/\d+/)?.[0] ? new Date(Number(p.Date.match(/\d+/)[0])).toISOString().slice(0,10) : '?'} ← account "${pay.Account?.Name}"`);
      await sleep(1100);
    }
    await sleep(1100);
  }
