...
async function main() {
  TK = await tokens();
  const bills = await prisma.document.findMany({ where: { organizationId: ORG, type: 'BILL', config: { path: ['reference'], equals: 'BIPL-JPSG-INV-20260720-0094' } }, select: { name: true, config: true } });
  console.log(`bills ref'd to 0094: ${bills.length}`);
  for (const b of bills) {
    const c: any = b.config || {};
    if (!c.xeroBillId) { console.log(`  ${b.name}: NOT IN XERO`); continue; }
    const g = await xero('GET', `/Invoices/${c.xeroBillId}`);
    const inv = g.Invoices[0];
    console.log(`  ${b.name}: ${inv.Status} $${inv.Total} acct=${inv.LineItems?.[0]?.AccountCode}`);
    await sleep(1100);
  }
