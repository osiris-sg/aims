...
async function main() {
  TK = await tokens();
  const d = await prisma.document.findFirst({ where: { organizationId: ORG, type: 'INVOICE', name: 'BI202607107 (a26c)' }, select: { id: true, config: true } });
  const c: any = d!.config || {};
  const g = await xero('GET', `/Invoices/${c.xeroInvoiceId}`);
  const inv = g.Invoices[0];
  console.log(`xero live: ${inv.Status} total=${inv.Total} due=${inv.AmountDue}`);
  const taken = await prisma.document.findFirst({ where: { organizationId: ORG, type: 'INVOICE', name: 'BI202607107' }, select: { id: true } });
  await prisma.document.update({
    where: { id: d!.id },
    data: {
      name: taken ? 'BI202607107 (a26c)' : 'BI202607107',
      status: 'pending_payment',
      config: { ...c, xeroStatus: inv.Status, xeroBalance: Number(inv.AmountDue), xeroAmountPaid: Number(inv.AmountPaid), xeroGross: Number(inv.Total) },
    },
  });
  console.log(`mirror fixed: balance=${inv.AmountDue}${taken ? '' : ', renamed to BI202607107'}`);
