...
async function main() {
  TK = await tokens();
  const b = await prisma.document.findFirst({ where: { organizationId: ORG, type: 'BILL', name: 'JP2605140022' }, select: { config: true } });
  const id = (b!.config as any).xeroBillId;
  const g = await xero('GET', `/Invoices/${id}`);
  const inv = g.Invoices[0];
  await sleep(1100);
  const lines = (inv.LineItems || []).map((l: any) =>
    Number(l.LineAmount) > 0
      ? { LineItemID: l.LineItemID, Description: l.Description, Quantity: 1.0, UnitAmount: 20.0, LineAmount: 20.0, AccountCode: l.AccountCode || '443', TaxType: 'NONE', TaxAmount: 0 }
      : { Description: l.Description });
  const up = await xero('POST', `/Invoices/${id}`, { Invoices: [{ InvoiceID: id, LineAmountTypes: 'NoTax', LineItems: lines }] });
  console.log('POST result total:', up.Invoices[0].Total, 'errors:', up.Invoices[0].HasErrors || 'none');
  await sleep(1100);
  const g2 = await xero('GET', `/Invoices/${id}`);
  console.log('re-fetched total:', g2.Invoices[0].Total, 'due:', g2.Invoices[0].AmountDue);
