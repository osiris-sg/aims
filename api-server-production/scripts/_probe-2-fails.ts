...
async function main() {
  ...
  for (const name of ['JP2604300061', 'JP2604290118']) {
    ...
    console.log(`${name}: AIMS total=${c.totalAmount} paid=${c.amountPaid} · Xero status=${inv?.Status} total=${inv?.Total} due=${inv?.AmountDue} paid=${inv?.AmountPaid} credits=${JSON.stringify((inv?.Payments || []).map((p: any) => p.Amount))}`);
    await sleep(1100);
  }
