...
async function main() {
  ...
  // our pushed pass drafts only (not the accountant's authorised disposals)
  ...
  console.log(`pass drafts to update: ${targets.length}`);
  let ok = 0, failed = 0;
  for (let i = 0; i < targets.length; i += 40) {
    ...
    const res = await xero('POST', '/Invoices?SummarizeErrors=false', {
      Invoices: chunk.map(d => ({ InvoiceID: (d.config as any).xeroInvoiceId, Reference: `${d.name} (JP Pass Application)` })),
    });
  ...
  console.log(`DONE refs set: ${ok} failed: ${failed}`);
