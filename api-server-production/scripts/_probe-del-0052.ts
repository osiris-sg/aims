...
async function xero(method: string, path: string, body?: any) {
  for (let i = 0; i < 6; i++) {
    ...
    if (!res.ok) throw new Error(`${res.status}: ${JSON.stringify(json).slice(0, 2000)}`);
    return json;
  }
...
async function main() {
  TK = await tokens();
  const g = await xero('GET', '/Invoices/d0f66e0d-8919-495a-a83a-cbd45950c9fc');
  const inv = g?.Invoices?.[0];
  console.log('status=', inv?.Status, 'number=', inv?.InvoiceNumber, 'total=', inv?.Total, 'lines=', (inv?.LineItems || []).length);
  console.log('line0=', JSON.stringify(inv?.LineItems?.[0]));
  try {
    await xero('POST', '/Invoices/d0f66e0d-8919-495a-a83a-cbd45950c9fc', { Invoices: [{ InvoiceID: 'd0f66e0d-8919-495a-a83a-cbd45950c9fc', Status: 'DELETED' }] });
    console.log('delete ok');
  } catch (e: any) {
    const m = e.message.match(/ValidationErrors[^\]]*\]/g);
    console.log('DELETE ERR:', m ? m.join(' | ').slice(0, 800) : e.message.slice(0, 1200));
  }
