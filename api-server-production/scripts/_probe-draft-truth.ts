...
async function main() {
  TK = await tokens();
  for (const name of ['JP2607200115', 'JP2607140094', 'JP2607290103']) {
    const b = await prisma.document.findFirst({ where: { organizationId: ORG, type: 'BILL', name }, select: { config: true } });
    const c: any = b!.config || {};
    if (!c.xeroBillId) { console.log(`${name}: no xero link`); continue; }
    const g = await xero('GET', `/Invoices/${c.xeroBillId}`);
    const inv = g.Invoices[0];
    console.log(`${name}: XERO says ${inv.Status} $${inv.Total} "${inv.InvoiceNumber}" | AIMS mirror says ${c.xeroStatus}`);
    await sleep(1100);
  }
