/** Xero ACCPAY UI never shows the API Reference field — make the recharge
 *  linkage visible by appending it to each linked draft's line description. */
...
async function main() {
  ...
  console.log(`linked bills to update in Xero: ${bills.length}`);
      ...
      Invoices: chunk.map((b: any) => {
        ...
        return {
          ...
          LineItems: (c.lines || []).map((li: any) => ({
            Description: `${li.description} · Recharge: ${ref}`,
  ...
  console.log(`descriptions updated in Xero: ${ok} failed: ${failed}`);
}
