...
async function main() {
  TK = await tokens();
  const res = await xero('GET', `/Invoices?where=${encodeURIComponent('InvoiceNumber=="BI202607065"')}`);
  for (const inv of res.Invoices || []) {
    console.log(`${inv.InvoiceID} status=${inv.Status} total=${inv.Total} due=${inv.AmountDue} updated=${inv.UpdatedDateUTC}`);
  }
