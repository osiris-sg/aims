...
async function main() {
  TK = await tokens();
  const bills = await prisma.document.findMany({ where: { organizationId: ORG, type: 'BILL', config: { path: ['reference'], equals: 'BIPL-JPSG-INV-20260720-0094' } }, select: { id: true, name: true, config: true } });
  let ok = 0, failed = 0;
  for (const b of bills) {
    const c: any = b.config || {};
    try {
      const up = await xero('POST', `/Invoices/${c.xeroBillId}`, { Invoices: [{ InvoiceID: c.xeroBillId, Status: 'AUTHORISED' }] });
      const u = up.Invoices[0];
      if (u.HasErrors) { failed++; console.log(`x ${b.name}: ${u.ValidationErrors?.[0]?.Message}`); continue; }
      await prisma.document.update({ where: { id: b.id }, data: { config: { ...c, xeroStatus: 'AUTHORISED' } } });
      ok++;
      await sleep(1100);
    } catch (e: any) { failed++; console.log(`x ${b.name}: ${e.message.slice(0, 100)}`); }
  }
  console.log(`approved ${ok}/9, failed ${failed}`);
