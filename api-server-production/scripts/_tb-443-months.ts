...
async function main() {
  ...
  const r = await xero('GET', `/Reports/TrialBalance?date=${d}`);
  const rows = r.Reports?.[0]?.Rows || [];
  const walk = (rs) => rs.flatMap((x) => [x, ...(x.Rows ? walk(x.Rows) : [])]);
  for (const row of walk(rows)) {
    const cells = row.Cells || [];
    if (/443/.test(cells[0]?.Value || '')) console.log(d, '→', cells.map((c) => c.Value).join(' | '));
  }
  await sleep(1100);
}
const r = { Reports: [] };
  ...
  for (const row of walk(rows)) {
    ...
    if (/443|JP Pass/.test(label)) console.log(cells.map((c: any) => c.Value).join(" | "));
  }
