...
async function main() {
  ...
  for (const name of ['JP2607140112', 'JP2607140113', 'JP2607140114']) {
    const doc = await prisma.document.findFirst({ where: { organizationId: ORG, type: 'BILL', name }, select: { config: true } });
    const id = (doc?.config as any)?.xeroBillId;
    if (!id) { console.log(`x ${name}: no xeroBillId`); continue; }
    const res = await xero('POST', `/Invoices/${id}`, { Invoices: [{ InvoiceID: id, InvoiceNumber: `${name} \u00b7 ${REF}` }] });
    const inv = res?.Invoices?.[0];
    console.log(inv?.HasErrors ? `x ${name}: ${inv.ValidationErrors?.[0]?.Message}` : `ok ${name} -> "${name} \u00b7 ${REF}"`);
    await sleep(1100);
  }
