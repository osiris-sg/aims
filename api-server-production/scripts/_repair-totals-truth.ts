...
async function main() {
  TK = await tokens();
  // all ref'd bills whose AIMS totalAmount disagrees with their (verified) payment
  ...
  let fixed = 0, aimsOnly = 0;
  for (const b of bills) {
    const c: any = b.config || {};
    if (!/^(BIPL-JPSG-INV|JPINV-|BI\d)/.test(c.reference || '')) continue;
    const bp = await prisma.billPayment.findFirst({ where: { organizationId: ORG, billId: b.id }, select: { amount: true } });
    if (!bp) continue;
    const truth = Number(bp.amount);
    const aims = Number(c.totalAmount || 0);
    if (Math.abs(aims - truth) < 0.005) continue;
    // fix AIMS
    const lines = (c.lines || []).map((l: any) => ({ ...l, amount: truth, unitPrice: truth }));
    await prisma.document.update({ where: { id: b.id }, data: { config: { ...c, totalAmount: truth, subtotal: truth, taxAmount: 0, amountsAre: 'NO_TAX', amountPaid: truth, xeroGross: truth, lines } } });
    aimsOnly++;
    // fix Xero if линed
    if (c.xeroBillId) {
      try {
        ...
        await sleep(1100);
        if (Math.abs(Number(inv.Total) - truth) > 0.005 && !['VOIDED', 'DELETED'].includes(inv.Status)) {
          const xl = (inv.LineItems || []).map((l: any) =>
            Number(l.LineAmount) > 0
              ? { LineItemID: l.LineItemID, Description: l.Description, Quantity: 1, UnitAmount: truth, AccountCode: l.AccountCode || '443', TaxType: 'NONE' }
              : { Description: l.Description });
          const up = await xero('POST', `/Invoices/${inv.InvoiceID}`, { Invoices: [{ InvoiceID: inv.InvoiceID, LineAmountTypes: 'NoTax', LineItems: xl }] });
          const u = up.Invoices[0];
          console.log(u.HasErrors ? `x ${b.name}: ${u.ValidationErrors?.[0]?.Message}` : `ok ${b.name}: ${aims} → ${truth} (xero total ${u.Total}, due ${u.AmountDue})`);
          if (!u.HasErrors) fixed++;
          await sleep(1100);
        }
      } catch (e: any) { console.log(`x ${b.name}: ${e.message.slice(0, 100)}`); }
    }
  }
  console.log(`DONE: aims-corrected=${aimsOnly}, xero-corrected=${fixed}`);
