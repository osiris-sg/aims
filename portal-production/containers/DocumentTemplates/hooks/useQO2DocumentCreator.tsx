...
export default function useQO2DocumentCreator() {
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      customerId: "",
      items: [{ itemNo: 1, itemDesc: "", unitRateOneYearOrLess: "", unitRateMoreThanOneYear: "", salesPrice: "" }],
  ...
  const addNewLine = () => {
    append({ itemNo: fields.length + 1, itemDesc: "", unitRateOneYearOrLess: "", unitRateMoreThanOneYear: "", salesPrice: "" });
  };
  ...
  useEffect(() => {
    ...
    if (documentId && document?.config) {
      ...
      // Preload org logo for new documents
      setValue("logo", [{ data: organization.logo }], { shouldDirty: true });
    }
  }, [documentId, document?.config, reset, setValue, document?.organization, organization?.logo]);
