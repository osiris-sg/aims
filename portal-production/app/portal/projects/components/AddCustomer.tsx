...
export default function AddCustomer({ open, onClose, onSuccess, customerId, isEditMode = false }: AddCustomerProps) {
  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      if (response.success) {
        ...
        reset();
        onClose();
        onSuccess(response.data); // Pass newly created customer
      }
    } catch (error) {
  ...
  return (
    <Drawer anchor="right" open={open} onClose={onClose} sx={{ "& .MuiDrawer-paper": { width: { xs: "100%", sm: "400px" }, backgroundColor: "background.paper", backgroundImage: "none", borderLeft: 1, borderColor: "divider" } }}>
