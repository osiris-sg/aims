...
import { expandUploadFiles } from "@/helpers/uploadExpand";
...
export default function DocumentUploadDialog({ open, onClose, documentType, documentLabel, onCreated }: Props) {
  ...
  // contents queued individually (shared helpers/uploadExpand — guru 2026-07-26).
  const addFiles = async (list: FileList | File[] | null) => {
    const accepted = await expandUploadFiles(list);
            ...
            <List dense sx={{ border: 1, borderColor: "divider", borderRadius: 1, maxHeight: 220, overflow: "auto", py: 0 }}>
              {results.map((r, i) => (
                ...
                >
                  ...
                  <ListItemText primary={r.name} secondary={r.ok ? "Created" : r.error} />
                </ListItem>
              ))}
            </List>
          )}
