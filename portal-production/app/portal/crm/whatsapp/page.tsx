...
export default function WhatsAppPage() {
  ...
  // Whether the in-flight signup used the Coexistence flow (existing WhatsApp
  // Business app number) — the backend must skip phone registration for those.
    ...
    async (code: string) => {
      ...
      try {
        await request("/whatsapp/onboard", {
          method: "POST",
          body: JSON.stringify({ code, wabaId, phoneNumberId, coexistence: coexistenceRef.current }),
        });
    ...
    (coexistence: boolean) => {
      ...
      window.FB.login(
        ...
        },
        {
          ...
          extras: {
            setup: {},
            // Coexistence: onboard an existing WhatsApp Business APP number —
            // the phone keeps working and chats sync both ways.
          ...
          <Stack spacing={2} alignItems="flex-start">
            ...
            <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
              ...
              <Button
                ...
                disabled={!sdkReady || connecting}
                onClick={() => launchSignup(true)}
              >
                Connect existing Business app number
              </Button>
            </Stack>
            <Typography variant="caption" color="text.secondary">
              Use the second option if the number currently lives in the WhatsApp Business app on a phone — the app
              keeps working and chats sync (Coexistence). Requires WhatsApp Business app v2.24.17+.
            </Typography>
            {!sdkReady && (
              <Typography variant="caption" color="text.secondary">
