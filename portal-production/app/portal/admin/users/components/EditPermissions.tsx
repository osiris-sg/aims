...
export default function EditPermissions({ open, onClose, role, onPermissionsUpdated }: Props) {
  ...
  return (
    <Drawer anchor="right" open={open} onClose={onClose}>
      <Box sx={{ width: { xs: "100vw", sm: 500 }, p: 3, height: "100%", display: "flex", flexDirection: "column", backgroundColor: "background.paper" }}>
              ...
              <Box key={resource} sx={{ mb: 3 }}>
                ...
                <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
                  {perms.map((permission) => (
                    <PermissionCheckbox key={permission.id} permission={permission} checked={selectedPermissions.includes(permission.id)} onChange={() => handleTogglePermission(permission.id)} />
                  ))}
                </Box>
              </Box>
            ))}
