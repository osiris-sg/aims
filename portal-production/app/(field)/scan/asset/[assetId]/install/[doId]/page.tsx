...
// Best-effort one-shot GPS fix for the installation point. Resolves to null on
// ANY failure — permission denied, no signal, timeout, or the API being
// unavailable — so acknowledgement is never blocked by GPS. latitude/longitude
// may persist blank.
...
export default function InstallationAckPage() {
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        ...
        const key = await uploadImage({ blob: compressedBlob, folderName: "do-install", token });
  ...
  const continueToSign = async () => {
    ...
    try {
      ...
      // Capture a one-shot GPS fix for the installation point so the coordinates
      // persist on the MSR. Best-effort only: capturePosition resolves null on
    ...
    <Box sx={{ p: 3, display: "flex", flexDirection: "column", gap: 3 }}>
      ...
      <Box>
        <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 1 }}>
          <Typography variant="subtitle2">Proof of installation ({photos.length})</Typography>
